...
function Presence() {
  this.client = createClient({ url: 'rediss://rtc-redis-test-w0htzp.serverless.apse1.cache.amazonaws.com:6379' });
...
Presence.prototype.upsert = async function(connectionId, meta) {
  try {
    await this.client.hSet('presence', connectionId, JSON.stringify({
      meta: meta,
      when: Date.now()
    }));
  } catch (err) {
